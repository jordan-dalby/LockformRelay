...
export function createApp(deps: Deps): Hono {
  ...
  // app in the browser. The webhook endpoint is server-to-server (no Origin
  // header) and is unaffected by CORS.
  app.use('*', cors({
    origin: (origin) => (deps.env.allowedOrigins.includes(origin) ? origin : null),
